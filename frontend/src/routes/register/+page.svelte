<script lang="ts">
	...
	async function handleRegister() {
		...
		try {
			...
			if (response.success && response.data) {
				...
				if (response.error) {
					try {
						...
						// 🔐 Parse and sanitize validation errors
						validationErrors = parseValidationErrors(errorData);
						errorMessage = getDisplayError(validationErrors);
					} catch {
						// Fallback: sanitize the raw error message
			...
			isLoading = false;
			// 🔐 Always clear password on form submission (success or failure)
			password = '';
			confirmPassword = '';
		}
	}
