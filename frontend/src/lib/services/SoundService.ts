...
class SoundServiceClass {
	...
	public init(): void {
		...
		try {
			...
			if (this.sounds.size > 0) {
				logger.debug('[SoundService] Initialized with', this.sounds.size, 'sound(s)');
			}
		} catch (error) {
			logger.error('[SoundService] Initialization error:', error);
		}
	}
	...
	private loadConfig(): SoundConfig {
		...
		try {
			...
			logger.warn('[SoundService] Failed to load config:', error);
		}
	...
	private saveConfig(): void {
		...
		try {
			...
			logger.warn('[SoundService] Failed to save config:', error);
		}
	}
