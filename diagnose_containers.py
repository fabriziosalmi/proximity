...
async def main():
    ...
    for app in db_apps:
        print(f"   - {app.hostname} (LXC {app.lxc_id}, status: {app.status}, owner: {app.owner_id or 'None'})")
    print()
    ...
    try:
        ...
        for node_name, container in all_containers:
            ...
            name = container.get('name', 'N/A') if isinstance(container, dict) else getattr(container, 'name', 'N/A')
            ...
            try:
                ...
                if vmid in db_lxc_ids:
                    ...
                    print(f"   ⚠️  NOT IN DATABASE - Orphan container!")
                    print(f"   → This container exists in Proxmox but is not tracked by Proximity")
                    print(f"   → Possible causes:")
                    print(f"      1. Created manually in Proxmox")
                    print(f"      2. Failed deployment that didn't save to DB")
                    print(f"      3. Database was cleared/reset")
