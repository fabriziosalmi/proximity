...
def test_sentry_with_full_context():
    ...
    try:
        ...
        if not test_user:
            test_user = User.objects.create_user(
                username="sentry_test_user", email="sentry.test@proximity.local", password="test123"
            )
    ...
    logger.info(
        ...
        extra={"log_source": "sentry_test", "test_type": "integration", "user": "sentry_test_user"},
    )
    print("   ✅ Log sent via Python logging")

    logger.warning(
        "Test warning message", extra={"log_source": "sentry_test", "severity": "medium"}
    )
    print("   ✅ Warning log sent")

    logger.error(
        "Test error log (not an exception)", extra={"log_source": "sentry_test", "severity": "high"}
    )
