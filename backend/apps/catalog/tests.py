...
VALID_APP_DATA = {
    ...
    "docker_compose": {
        "version": "3.8",
        "services": {"test": {"image": "test:latest", "environment": {}, "restart": "always"}},
    },
    "ports": [8080],
    "volumes": [],
    "environment": {},
...
ANOTHER_VALID_APP = {
    ...
    "version": "2.0.0",
    "description": "A database application in a different category",
...
class CatalogAPITestCase(TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        # Create test admin user for authenticated endpoints
        ...
        # Update the module-level catalog_service variable in the api module
        import apps.catalog.api as api_module

        api_module.catalog_service = self.service
    ...
    def test_reload_functionality(self):
        """Test the reload functionality (reload endpoint requires auth)."""
        # Test the reload functionality directly on the service
        # The API endpoint is admin-only and tested separately
        initial_count = len(self.service.get_all_apps())
        self.assertEqual(initial_count, 2)

        # Verify reload works
        self.service.reload()

        # After reload, should still have 2 apps
        reloaded_count = len(self.service.get_all_apps())
        self.assertEqual(reloaded_count, 2)
