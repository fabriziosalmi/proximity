...
class NodeSelectionLogicTest(TransactionTestCase):
    ...
    def test_node_selection_prefers_opti2_over_pve(self):
        ...
        self.assertNotEqual(
            app.node.name, "pve", "System incorrectly selected 'pve' node which is unreachable!"
        )

        print(f"✅ SUCCESS: System correctly selected node '{app.node.name}'")
        print(
            f"   pve memory free: {self.pve_node.memory_total - self.pve_node.memory_used:,} bytes"
        )
        print(
            f"   opti2 memory free: {self.opti2_node.memory_total - self.opti2_node.memory_used:,} bytes"
        )
