...
class PortManagerService:
    ...
    def get_port_range_usage(self) -> dict:
        ...
        public_used = Application.objects.filter(
            public_port__gte=self.PUBLIC_PORT_START,
            public_port__lte=self.PUBLIC_PORT_END
        ).count()
        
        internal_used = Application.objects.filter(
            internal_port__gte=self.INTERNAL_PORT_START,
            internal_port__lte=self.INTERNAL_PORT_END
        ).count()
