...
class ApplicationCreate(BaseModel):
    ...
    hostname: str = Field(..., min_length=3, max_length=63, description="Unique hostname (RFC 1123 format)")
    ...
    network_cidr: Optional[str] = Field(None, description="Network CIDR for container (e.g., 10.0.0.0/24)")
...
class ApplicationClone(BaseModel):
    ...
    new_hostname: str = Field(..., min_length=3, max_length=63, description="Hostname for the cloned application (RFC 1123 format)")
...
class ApplicationAdopt(BaseModel):
    ...
    port_to_expose: Optional[int] = Field(None, ge=1, le=65535, description="Optional: specific port to expose (1-65535)")
