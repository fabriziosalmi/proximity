...
def select_best_node(hosts):
    ...
    online_nodes = ProxmoxNode.objects.filter(status="online", host__is_active=True).select_related(
        "host"
    )
...
def test_smart_node_selection_chooses_best_node(django_user_model):
    ...
    assert new_app.node is not None, "Application was created but no node was assigned"
    ...
    assert new_app.node != "offline-node", "System incorrectly selected an offline node!"

    # 7. Verify it did NOT select pve (less free memory)
    assert new_app.node != "pve", (
        "System selected 'pve' (2GB free) instead of 'opti2' (8GB free). "
        "Memory-based selection is not working correctly."
    )
    ...
    print(
        f"   Node memory: {(opti2_node.memory_total - opti2_node.memory_used) / 1024**3:.1f}GB free"
    )
...
def test_node_selection_excludes_inactive_host():
    ...
    online_nodes = ProxmoxNode.objects.filter(status="online", host__is_active=True).order_by(
        "-memory_total"
    )

    # Verify only active host node is returned
    assert online_nodes.count() == 1, f"Expected 1 eligible node, found {online_nodes.count()}"
    assert (
        online_nodes.first().name == "active-node"
    ...
    assert "inactive-node" not in [
        n.name for n in online_nodes
