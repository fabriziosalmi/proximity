...
def create_application(request, payload: ApplicationCreate):
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, IntegrityError):
                ...
                if public_port and internal_port:
                    ...
                    logger.info(f"[API] Released ports {public_port}/{internal_port} due to hostname conflict")
                raise HttpError(409, f"Hostname '{payload.hostname}' already exists (race condition detected)")
            else:
        ...
        logger.error(f"[API] ❌ Unexpected error during deployment creation: {str(e)}", exc_info=True)
        if public_port and internal_port:
            port_manager.release_ports(public_port, internal_port)
