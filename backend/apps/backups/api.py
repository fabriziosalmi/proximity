...
def list_app_backups(request, app_id: str):
    ...
    # Convert to schema
    backup_schemas = [
        BackupSchema.model_validate(backup)
...
def create_app_backup(
    ...
    if app.status in ['removing', 'error']:
        return (
            400,
            {
                "error": f"Cannot backup application in '{app.status}' state"
            }
        )
    ...
    if in_progress:
        return (
            409,
            {
                "error": "A backup or restore operation is already in progress for this application"
            }
        )
...
def restore_from_backup(request, app_id: str, backup_id: int):
    ...
    if backup.status != 'completed':
        return (
            400,
            {
                "error": f"Cannot restore from backup in '{backup.status}' state. Only completed backups can be restored."
            }
        )
...
def delete_backup(request, app_id: str, backup_id: int):
    ...
    if backup.status in ['creating', 'restoring', 'deleting']:
        return (
            409,
            {
                "error": f"Cannot delete backup in '{backup.status}' state"
            }
        )
