...
class TestBackupAPIEndpoints:
    ...
    def test_list_backups_with_data(self, auth_client, sample_application, sample_backup):
    ...
    def test_list_backups_other_user_app(self, auth_client, other_user_application):
    ...
    def test_create_backup_success(self, auth_client, sample_application):
        ...
        with patch("apps.backups.api.create_backup_task") as mock_task:
            ...
            response = auth_client.post(
                ...
                json={"backup_type": "snapshot", "compression": "zstd"},
    ...
    def test_create_backup_app_in_error_state(self, auth_client, sample_application):
    ...
    def test_create_backup_already_in_progress(self, auth_client, sample_application):
    ...
    def test_get_backup_details(self, auth_client, sample_application, sample_backup):
        """Test getting backup details."""
        response = auth_client.get(f"/api/apps/{sample_application.id}/backups/{sample_backup.id}")
        assert response.status_code == 200
    ...
    def test_get_backup_details_not_found(self, auth_client, sample_application):
        ...
        response = auth_client.get(f"/api/apps/{sample_application.id}/backups/99999")
        assert response.status_code == 404

    def test_restore_backup_success(self, auth_client, sample_application, sample_backup):
    ...
    def test_restore_backup_not_completed(self, auth_client, sample_application):
    ...
    def test_restore_backup_already_in_progress(
        self, auth_client, sample_application, sample_backup
    ):
        """Test that restore cannot start if operation already in progress."""
        # Create another backup in progress
        Backup.objects.create(
            application=sample_application, file_name="other.tar.zst", status="restoring"
        )
    ...
    def test_delete_backup_success(self, auth_client, sample_application, sample_backup):
    ...
    def test_delete_backup_in_progress(self, auth_client, sample_application):
        """Test that backups in progress cannot be deleted."""
        backup = Backup.objects.create(
            application=sample_application, file_name="test.tar.zst", status="creating"
        )

        response = auth_client.delete(f"/api/apps/{sample_application.id}/backups/{backup.id}")
        # Verify conflict when backup is in progress
        assert response.status_code == 409

    def test_get_backup_stats(self, auth_client, sample_application, sample_backup):
        ...
        response = auth_client.get(f"/api/apps/{sample_application.id}/backups/stats")
        assert response.status_code == 200
