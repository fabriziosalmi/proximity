...
def create_backup_task(
    self, application_id: str, backup_type: str = "snapshot", compression: str = "zstd"
):
    """
    ...
    try:
        ...
        logger.info(
            f"Starting backup for app {app.name} (LXC {app.lxc_id}), " f"backup_id={backup.id}"
        )
        ...
        logger.info(f"Backup completed successfully: {backup.file_name} " f"({backup.size_mb} MB)")
        ...
        return {"success": False, "backup_id": backup.id if backup else None, "error": str(e)}
...
def restore_backup_task(self, backup_id: int):
    ...
    try:
        ...
        try:
            backup = Backup.objects.select_related("application", "application__host").get(
                id=backup_id
            )
        ...
        try:
            ...
            # Log if we can't revert status, but don't fail the task
            logger.error(
                f"Failed to revert status after restore error: {revert_error}", exc_info=True
            )
...
def delete_backup_task(self, backup_id: int):
    ...
    try:
        ...
        logger.info(
            f"Deleting backup {backup.file_name} for app {app.name} " f"(backup_id={backup_id})"
        )
        ...
        try:
            proxmox.delete_backup_file(
                node_name=app.node, storage=backup.storage_name, backup_file=backup.file_name
            )
            ...
            if "not found" not in error_msg and "does not exist" not in error_msg:
                raise
            logger.warning("Backup file not found in storage, will delete record anyway")
        ...
        return {"success": True, "backup_id": backup_id, "message": "Backup deleted successfully"}
        ...
        try:
            ...
            # Log if we can't revert status, but don't fail the task
            logger.error(
                f"Failed to revert backup status after deletion error: {revert_error}",
                exc_info=True,
            )
        ...
        try:
            ...
            # Log if we can't revert status, but don't fail the task
            logger.error(
                f"Failed to revert backup status after unexpected error: {revert_error}",
                exc_info=True,
            )
