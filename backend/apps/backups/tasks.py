...
def restore_backup_task(self, backup_id: int):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to revert status after restore error: {revert_error}", exc_info=True)
...
def delete_backup_task(self, backup_id: int):
    ...
    try:
        ...
        try:
            ...
            # Log if we can't revert status, but don't fail the task
            logger.error(f"Failed to revert backup status after deletion error: {revert_error}", exc_info=True)
        ...
        try:
            ...
            # Log if we can't revert status, but don't fail the task
            logger.error(f"Failed to revert backup status after unexpected error: {revert_error}", exc_info=True)
