...
class ProxmoxService:
    ...
    def configure_lxc_for_docker(self, node_name: str, vmid: int) -> None:
        ...
        try:
            ...
            logger.info(
                f"Configured LXC {vmid} for Docker (AppArmor: unconfined, Capabilities: all)"
            )
    ...
    def delete_lxc(self, node_name: str, vmid: int, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if force:
                ...
                except (ProxmoxError, Exception) as e:
                    # Ignore errors if container already stopped - just log and continue
                    logger.warning(
                        f"Could not stop container {vmid} before deletion (may already be stopped): {e}"
                    )
    ...
    def create_snapshot(
        ...
        try:
            ...
            task_upid = (
                ...
                .lxc(vmid)
                .snapshot.post(
                    snapname=snapname, description=description or "Temporary snapshot for cloning"
                )
            )
    ...
    def clone_lxc(
        ...
        try:
            ...
            if is_running and full:
                logger.info(
                    f"[Step 2/4] Creating temporary snapshot '{snapshot_name}' for zero-downtime clone..."
                )
                ...
                logger.info(
                    f"[Step 2/4] Skipping snapshot creation (running={is_running}, full={full})"
                )

            # Step 3/4: Perform the clone operation
            logger.info(
                f"[Step 3/4] Cloning LXC {source_vmid} → {new_vmid} (hostname: {new_hostname})..."
            )
            ...
            logger.info("[Step 3/4] ✓ Clone completed successfully")
    ...
    def update_lxc_config(self, node_name: str, vmid: int, **config_params) -> Dict[str, Any]:
        """
    ...
    def wait_for_task(
        self, node_name: str, upid: str, timeout: int = 600, poll_interval: int = 2
    ) -> Dict[str, Any]:
        """
            ...
            while True:
                ...
                if status.get("status") == "stopped":
                    ...
                    if exitstatus == "OK":
                        ...
                        raise ProxmoxError(f"Task {upid} failed with status: {exitstatus}")
    ...
    def create_lxc_backup(
        ...
        try:
            ...
            if mode not in valid_modes:
                raise ProxmoxError(f"Invalid backup mode '{mode}'. Must be one of: {valid_modes}")
            ...
            backups = client.nodes(node_name).storage(storage).content.get(content="backup")

            # Find the most recent backup for this VMID
            vmid_backups = [b for b in backups if f"lxc-{vmid}-" in b.get("volid", "")]

            if not vmid_backups:
                raise ProxmoxError(f"Backup file not found for LXC {vmid}")

            # Sort by creation time and get the latest
            latest_backup = sorted(vmid_backups, key=lambda x: x.get("ctime", 0), reverse=True)[0]
            ...
            logger.info(f"Backup completed successfully: {file_name} ({size} bytes)")
    ...
    def restore_lxc_backup(
        ...
        try:
            ...
            logger.info(
                f"Restoring LXC {vmid} from backup {backup_file} " f"(volid={volid}, force={force})"
            )

            # Trigger restore operation
            task_upid = client.nodes(node_name).lxc.post(
                vmid=vmid, ostemplate=volid, restore=1, force=int(force), storage=storage
            )
            ...
            return {"task": task_status, "vmid": vmid, "backup_file": backup_file}

        except Exception as e:
            if isinstance(e, ProxmoxError):
                raise
            raise ProxmoxError(f"Failed to restore LXC {vmid} from {backup_file}: {e}")

    def delete_backup_file(self, node_name: str, storage: str, backup_file: str) -> bool:
        """
    ...
    def list_backups(
        self, node_name: str, storage: str = "local", vmid: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if vmid is not None:
                backups = [b for b in backups if f"lxc-{vmid}-" in b.get("volid", "")]
    ...
    def _execute_ssh_command(
        ...
        Prefers key-based authentication when available.

        Args:
            host: SSH host address
            port: SSH port (default 22)
            username: SSH username
            password: SSH password (fallback auth)
        ...
        try:
            ...
            # Load system's known_hosts file for proper host key verification
            ssh.load_system_host_keys()
            # Fall back to warning policy if host key not in known_hosts
            # This is safer than AutoAddPolicy but still operational
            ...
            if key_filename:
                ...
                try:
                    ...
                    logger.debug(f"SSH authentication successful using key: {key_filename}")
                except (paramiko.AuthenticationException, FileNotFoundError):
                    # Fall back to password authentication
                    logger.warning("Key-based auth failed, falling back to password auth")
    ...
    def execute_in_container(
        ...
        try:
            ...
            ssh_key_path = host.ssh_key_path  # Optional SSH key for key-based auth

            logger.debug(f"Executing in LXC {vmid} on node {node_name}: {command}")

            # Build pct exec command with proper shell escaping
            # Use shlex.quote to prevent command injection
            pct_command = f"pct exec {vmid} -- {shlex.quote(command)}"

            logger.debug(f"Full pct command: {pct_command}")

            # Execute via SSH with optional key-based authentication
            stdout, stderr, exit_code = self._execute_ssh_command(
                host=ssh_host,
                port=ssh_port,
                username=ssh_username,
            ...
            logger.debug(
                f"Command output: {stdout[:200]}..."
                if len(stdout) > 200
                else f"Command output: {stdout}"
            )
    ...
    def discover_unmanaged_lxc(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            managed_vmids = set(
                Application.objects.filter(lxc_id__isnull=False).values_list("lxc_id", flat=True)
            )
            ...
            logger.info(
                f"Discovery complete: Found {len(unmanaged_containers)} unmanaged containers"
            )
