...
class ProxmoxHost(models.Model):
    ...
    name = models.CharField(
        max_length=100, unique=True, help_text="Friendly name for this Proxmox host"
    )
    ...
    user = models.CharField(
        max_length=100, default="root@pam", help_text="Proxmox user (e.g., root@pam)"
    )
    password = EncryptedCharField(max_length=500, help_text="Encrypted password for Proxmox API")
    ...
    verify_ssl = models.BooleanField(default=False, help_text="Verify SSL certificate")

    # Status and metadata
    is_active = models.BooleanField(default=True, help_text="Enable/disable this host")
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="proxmox_hosts"
    )
