...
class NetworkSettingsRequest(BaseModel):
    ...
    default_gateway: str = Field(
        ..., pattern=r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$", description="Gateway IPv4 address"
    )
    default_dns_primary: str = Field(..., description="Primary DNS server")
    default_dns_secondary: Optional[str] = Field(
        None, description="Secondary DNS server (optional)"
    )
    default_bridge: str = Field(
        ..., min_length=1, max_length=50, description="Network bridge name (e.g., vmbr0)"
    )
