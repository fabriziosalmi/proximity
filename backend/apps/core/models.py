...
class User(AbstractUser):
    ...
    preferred_theme = models.CharField(
        max_length=50, default="rack_proximity", help_text="UI theme/skin preference"
    )
...
class SystemSettings(models.Model):
    ...
    default_cpu_cores = models.IntegerField(
        default=2, help_text="Default number of CPU cores for new LXC containers"
    )
    default_memory_mb = models.IntegerField(
        default=2048, help_text="Default memory allocation in MB for new LXC containers"
    )
    ...
    default_gateway = models.GenericIPAddressField(
        protocol="IPv4", default="10.0.0.1", help_text="Default gateway IP address"
    )
    default_dns_primary = models.GenericIPAddressField(
        protocol="both", default="8.8.8.8", help_text="Primary DNS server"
    )
    ...
    default_bridge = models.CharField(
        max_length=50, default="vmbr0", help_text="Default network bridge for containers"
    )
