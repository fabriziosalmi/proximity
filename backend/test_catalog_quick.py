...
def test_catalog_service():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create another app with different category
        another_app = VALID_APP_DATA.copy()
...
def test_api_endpoints():
    ...
    operations = [op.path for op in api.router.path_operations.values()]
    expected_paths = ['/', '/{app_id}', '/categories', '/search', '/stats', '/reload', '/category/{category}']
    
    for path in expected_paths:
        # Note: The actual paths may have prefixes, so we check if they end with expected path
        found = any(op.endswith(path) or path in op for op in operations)
        assert found, f"Path {path} not found in operations: {operations}"
    
    print(f"  ✓ All {len(expected_paths)} endpoints registered")
