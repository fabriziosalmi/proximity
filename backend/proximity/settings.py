...
CORS_ALLOWED_ORIGINS = [
    ...
    for origin in os.getenv(
        "CORS_ALLOWED_ORIGINS", "http://localhost:5173"  # Development default
    ).split(",")
    if origin.strip()
]

# Validate CORS origins - prevent wildcards in production
if not DEBUG:
    for origin in CORS_ALLOWED_ORIGINS:
        if "*" in origin:
            raise ValueError(f"Wildcard CORS origins not allowed in production: {origin}")
        if not origin.startswith(("http://", "https://")):
            raise ValueError(f"CORS origin must use http/https: {origin}")
