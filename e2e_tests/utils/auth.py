...
def cookie_login(
    ...
    base_url: str = "https://localhost:5173",
    initial_page: str = "/",
    ...
    if access_token:
        logger.info("  → Injecting JWT access_token into localStorage (AFTER page load)...")
        ...
        logger.info("  → Reloading page to apply token...")
    ...
    try:
        ...
        logger.info("  ✅ authStore initialized with cookie-based session!")
...
def programmatic_login(
    page: Page, auth_token: str, base_url: str = "https://localhost:5173", initial_page: str = "/"
) -> None:
    """
    ...
    logger.info("  ✓ Token injection script registered")
    ...
    logger.info("  → Reloading page to trigger authStore.init() with fresh token...")
    page.reload(wait_until="domcontentloaded")
    ...
    try:
        ...
        logger.info(
            f"  → Auth status: token={auth_status['hasToken']}, user={auth_status['hasUser']}, ready={auth_status['apiReady']}"
        )

        # Wait for the ready signal with a reasonable timeout
        # This is set by authStore.init() when it finds a valid session
        page.wait_for_selector(
            'body[data-api-client-ready="true"]', timeout=10000, state="attached"
        )
...
def wait_for_auth_ready(page: Page, timeout: int = 5000) -> None:
    ...
    try:
        page.wait_for_selector(
            'body[data-api-client-ready="true"]', timeout=timeout, state="attached"
        )
