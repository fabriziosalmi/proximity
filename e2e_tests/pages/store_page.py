...
class StorePage:
    ...
    DEPLOY_BUTTON = (
        'button:has-text("Deploy"), button:has-text("🚀 Deploy"), button:has-text("Install")'
    )
    ...
    MODAL_DEPLOY_BUTTON = (
        '.modal button:has-text("Deploy"), [role="dialog"] button:has-text("Deploy")'
    )
    MODAL_CANCEL_BUTTON = (
        '.modal button:has-text("Cancel"), [role="dialog"] button:has-text("Cancel")'
    )
    ...
    def confirm_deployment(self, wait_for_redirect: bool = True) -> "StorePage":
        ...
        if wait_for_redirect:
            # Wait for navigation to /apps page (frontend does goto('/apps') after successful deployment)
            # Use page.wait_for_url which is more reliable than expect().to_have_url()
            try:
                self.page.wait_for_url(
                    re.compile(r".*/apps/?$"), timeout=20000, wait_until="domcontentloaded"
                )
                print(f"  ✓ Redirected to: {self.page.url}")
            except Exception:
                print(f"  ⚠️  Navigation timeout or stayed on: {self.page.url}")
                # Take a screenshot for debugging
                self.page.screenshot(path="/tmp/deploy_redirect_failed.png")
                raise

        return self

    def deploy_app(
        self, app_name: str, hostname: str, wait_for_redirect: bool = True
    ) -> "StorePage":
        """
    ...
    def wait_for_apps_loaded(self, min_count: int = 1, timeout: int = 10000) -> "StorePage":
        ...
        # Use a longer timeout for the initial wait
        ...
        if actual_count < min_count:
            raise AssertionError(f"Expected at least {min_count} apps, but found {actual_count}")
