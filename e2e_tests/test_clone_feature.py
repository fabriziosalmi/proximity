...
def test_clone_application_lifecycle(
    ...
    print(
        f"DEBUG: auth_token = {deployed_app.get('auth_token', 'NOT FOUND')[:50] if deployed_app.get('auth_token') else 'NONE'}"
    )
    cookie_login(
        page, deployed_app["session_cookies"], base_url, access_token=deployed_app["auth_token"]
    )
    print("  ✅ ApiClient confirmed ready for authenticated requests")
    ...
    print("  ✓ Source app card visible")

    source_status = apps_page.get_app_status(source_hostname)
    assert source_status == "running", f"Expected source to be 'running', got '{source_status}'"
    ...
    print("  ✓ Clone button found and enabled")
    ...
    print("  ✅ Clone modal appeared\n")

    # ============================================================================
    # STEP 4: FILL CLONE MODAL
    # ============================================================================
    print("📍 STEP 4: Fill Clone Modal")
    print("-" * 80)
    ...
    submit_button.click()
    print("  ✓ Clicked 'Clone Application' button")

    # Wait for modal to close
    expect(clone_modal).to_be_hidden(timeout=5000)
    print("  ✅ Clone request submitted\n")
    ...
    assert clone_initial_status in [
        "cloning",
        "running",
    ], f"Expected 'cloning' or 'running', got '{clone_initial_status}'"
    print("  ✅ Clone visible with valid status\n")
    ...
    print("-" * 80)
    print("  ⏳ Waiting for clone to reach 'running' status...")
    print("  ⏳ Timeout: 10 minutes (LXC cloning can be slow)...")
    ...
    assert clone_status_final == "running", f"Clone should be running, got '{clone_status_final}'"
    ...
    print("  ✅ Both apps visible and running\n")
    ...
    print("-" * 80)
    print("  🎨 Testing 'Living Interface' features...")
    ...
    print("  ✓ 'is-flipped' class applied successfully")
    print("  ✅ Flip animation mechanism verified\n")
    ...
    print("Note: Cleanup handled by fixtures (deployed_app & unique_user)")
    print("=" * 80 + "\n")
...
def test_clone_application_workflow(
    test_page, unique_user: dict, proxmox_host: dict, base_url: str
):
    """
    ...
    # Wait for modal to close
    expect(clone_modal).to_be_hidden(timeout=5000)
    print("  ✅ CLONE INITIATED - Modal closed\n")
    ...
    print("  ✅ CLONE CARD VISIBLE with valid status\n")
    ...
    print("-" * 80)
    print("  ⏳ Waiting for clone to reach 'running' status...")
    print("  ⏳ This may take several minutes (LXC cloning + container start)...")
    ...
    source_status = apps_page.get_app_status(source_hostname)
    assert source_status == "running", f"Source app should still be running, got '{source_status}'"
    ...
    assert clone_status == "running", f"Clone app should be running, got '{clone_status}'"
    print(f"  ✓ Clone app: {clone_hostname} - Status: {clone_status}")

    print("  ✅ BOTH APPS RUNNING SUCCESSFULLY\n")
